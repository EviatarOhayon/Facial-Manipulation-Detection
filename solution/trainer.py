...
class Trainer:
    ...
    def train_one_epoch(self) -> tuple[float, float]:
        ...
        for batch_idx, (inputs, targets) in enumerate(train_dataloader):
            ...
            ''' avg. loss and accuracy calculation:'''
            nof_samples = self.batch_size * batch_idx + len(inputs)
            correct_labeled_samples += (pred.argmax(1) == targets).type(torch.float).sum().item() # Calculate number of correct predictions
            total_loss += loss.item() # Accumulate loss

            avg_loss = total_loss / (batch_idx+1)
            accuracy = correct_labeled_samples / nof_samples
            # NOTE : the accuracy is related to the correct_labeled_samples and not to the train_dataloader length - check it

            if batch_idx % print_every == 0 or batch_idx == len(train_dataloader) - 1:
